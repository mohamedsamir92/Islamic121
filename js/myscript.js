...
$("select#package").change(function() {
	...
	for (var i = 0; i < numberOfRows; ++i){
		$("#preference").append(
			...
			'<select class="form-control select days" name="Student[prefered_lesson_type_'+(i + 1)+']">'+
			'<option value="0">Quran</option>'+
			'<option value="1">Arabic</option>'+
			'</select>'+
			'</div>'+

			'<div class="col-md-1 col-xs-12">'+
			'<input type="number" min="30" max="120" value="30" class="form-control" name="Student[prefered_lesson_period_'+(i + 1)+']">'+
			'</div>'+
...
$('.timepicker_to').timepicker({ 'minuteStep' : 15 });

$('.timepicker_from').timepicker({ 'minuteStep' : 15 });
